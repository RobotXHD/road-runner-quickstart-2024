...
@Autonomous
@Disabled
public class abureli_program extends LinearOpMode {
    ...
    private DistanceSensor SenzorDistanta;
    ...
    public void runOpMode() {
        ...
        SenzorDistanta = (DistanceSensor) hardwareMap.get("senzor distanta 1");
        ...
        telemetry.addData("distanta", SenzorDistanta.getDistance(DistanceUnit.CM));
        telemetry.update();
        ...
        while(!isStarted()) {
            try {
                ...
                if (rectangleArea > minRectangleArea) {
                    //Then check the locat
                    //ion of the rectangle to see which barcode it is i
    ...
    public Thread Atunonom = new Thread(new Runnable(){
        ...
        Translatare(0,-68,0.3);

        DJL.setPower(-0.5);
        lastTime = System.currentTimeMillis();
        while(lastTime + 1900 > System.currentTimeMillis()){
        }
        DJL.setPower(0);

        Translatare(-50,130,0.5);
        ...
        Rotire(-100,0.5);
        Translatare(120, 55,0.6);
        Translatare(0, 145,0.6);
        ...
        Translatare(-40,-45,0.5);
        Rotire(155,0.5);
        Translatare(0,40,0.5);
        ...
        Translatare(0,-30,0.5);

        Rotire(-145, 0.7);
        lastTime = System.currentTimeMillis();
        while(lastTime + 250 > System.currentTimeMillis()){
        }

        Translatare(47,0,0.7);
        Translatare(5,180,0.7);
        }
